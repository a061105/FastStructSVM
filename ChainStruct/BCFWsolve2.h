...
class BCFWsolve{
	...
	Model* solve(){
		...
		for (Int i = 0; i < N; i++){
			...
			Seq* seq = data->at(n);
			Int yi = seq->labels[t];
			act_k_index[i].push_back(make_pair(yi, 0.0));
			for(Int k=0;k<K;k++)
				if( k!=yi )
					act_k_index[i].push_back(make_pair(k,0.0));
		}
		...
		for (Int i = 0; i < M; i++){
			...
			Seq* seq = data->at(n);
			Int yiyj = seq->labels[t]*K + seq->labels[t+1];
			act_kk_index[i].push_back(make_pair( yiyj, 0.0));
			for(Int kk=0;kk<K*K;kk++){
				if( kk != yiyj )
					act_kk_index[i].push_back(make_pair( kk, 0.0 ));
			}
		}
