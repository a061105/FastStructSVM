...
Model* solverBCFW(Param* param, Option* options){
	...
	for(int p = 0; p < options->num_pass; p++ ){
		...
		double test_error = average_loss( param, 0, n_train, param->data, param->labels, maxOracle, model_debug);
		...
		cerr << endl << "#pass=" << p << ", test_acc=" << (1-test_error) << ", time=" << (end-start-minus_time) << ", testPredTime=" << testPred_time  << ", d-obj=" << dual_obj(param, model->w, loss_term) << endl;
	}
