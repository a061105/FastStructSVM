function label = chain_oracle(param, model, xi, yi, ins_id)
...
% build score for chain graph using kernel model
theta_unary = zeros(num_states, num_vars);
for i=1:num_vars
	for j=1:num_states
		theta_unary(j,i) = kernel_inner_prod( param, model, xi.data(:,i), j-1, ins_id, i );
		%fprintf('(%d,%d,%d), kv=%g\n', ins_id-1, i-1, j-1, theta_unary(j,i));
	end
end
theta_pair = reshape( model.w_pair, [num_states num_states] );
