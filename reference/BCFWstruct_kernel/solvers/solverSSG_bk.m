...
model.offset = offset;
model.alpha = cell(n,1);
model.kernel_cache = cell(n,1);
for i = 1:n
	model.alpha{i} = cell(L(i),1);
	for j = 1:L(i)
		model.alpha{i}{j} = sparse(num_states,1);
	end
end
for i = 1:n
	model.kernel_cache{i} = cell(L(i),1);
	for j=1:L(i)
		model.kernel_cache{i}{j} = zeros(n,Lmax);
	end
end
...
for p=1:options.num_passes
	...
	for ii = 1:n
		for j=1:L(ii)
			...
			model.alpha{ii}{j} = (1-gamma)*model.alpha{ii}{j};
		end
	end
	for j=1:L(i)
		y1 = labels{i}(j)+1;
		y2 = ystar_i(j)+1;
		if y1 ~= y2
			alpha_s_y1_y2 = sparse([y1; y2],[1; 1],[alpha_s; -alpha_s], num_states,1);
			model.alpha{i}{j} = model.alpha{i}{j} + gamma*n*alpha_s_y1_y2;
		end
	end
	...
	model.w_pair = (1-gamma)*model.w_pair + gamma*n * w_s;
        ...
        if (options.do_weighted_averaging)
            ...
            for i=1:n
		    for j=1:L(i)
			    alpha_Avg{i}{j} = (1-rho)*alpha_Avg{i}{j} + rho*model.alpha{i}{j};
		    end
	    end
	...
	if (options.debug && k == debug_iter)
	    ...
	    model_debug.kernel_cache = model.kernel_cache;
