...
class BCFWsolve{
	...
	public:
		...
		BCFWsolve(Param* param){
			...
			act_alpha = new vector<pair<int, Float>>[N];
			for(Int i=0;i<N;i++){
				act_alpha[i].clear();
			}
			...
			memset(is_pos_label, 0, sizeof(bool)*K);
		...
		Model* solve(){
			...
			memset(beta_new, 0, sizeof(Float)*K*K);		
	...
	private:
		...
		void uni_subSolve(Int n, Float* alpha_new){ //solve i-th unigram factor
			...
			Instance* ins = data->at(n);
			Float Qii = Q_diag[n];
			vector<pair<int, Float>>* act_alpha_n = &(act_alpha[n]);
			...
			for(vector<pair<Int, Float>>::iterator it = act_alpha_n.begin(); it != act_alpha_n.end(); it++){
				Int k = it->first;
				alpha_new[k] = min( max( it->second - grad[k]/Qii, -C ), 0.0 );
			}
			
			for(vector<int>::iterator it=ins->labels.begin(); it!=ins->labels.end(); it++){
				Int k = *it;
				alpha_new[k] = min( max( it->second - grad[k]/Qii, 0.0 ), C );
			}
		}
